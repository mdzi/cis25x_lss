...
$ConfigPath="\\DC1\Software\IIS_NLB.xml"
#Install IIS & NLB per configuration file
$js = @()
foreach($ComputerName in $ComputerNames) {
$js += Start-Job -Command {
Install-WindowsFeature "Web-Default-Doc","Web-Dir-Browsing","Web-Http-Errors","Web-Static-Content","Web-Http-Logging","Web-Stat-Compression","Web-Filtering","Web-Mgmt-Console","NLB","RSAT-NLB" -ComputerName
...
$cluster = (New-NLBCluster -HostName $primaryHost -InterfaceName $interface -
ClusterName $ClusterAName -ClusterPrimaryIP $ClusterIP -SubnetMask $ClusterSN -
OperationMode "Multicast")
#Add a new node to the cluster
$cluster | Add-NLBClusterNode -NewNodeName $secondaryHost -NewNodeInterface
$interface
#Add port rules to the cluster
$cluster | Get-NLBClusterPortRule | Remove-NLBClusterPortRule -Force
foreach ($Port in $ClusterPOrts) {
$cluster | Add-NLBClusterPortRule -IP $ClusterFrom -Protocol TCP -StartPort
$Port -EndPort $Port -Mode "Multiple" -Affinity "Single" -Timeout 0
}
#Create an identification file
foreach($ComputerName in $ComputerNames) {
Invoke-Command -ComputerName $ComputerName -ScriptBlock { $Env:ComputerName |
Out-File C:\INETPUB\WWWROOT\default.htm -Encoding ASCII }
}
#Create DNS Records
Add-DnsServerResourceRecordA -Name $ClusterBaseName -IPv4Address $ClusterIP -
ZoneName $DNSZone
Add-DnsServerResourceRecordCName -Name "www" -HostNameAlias $ClusterBaseName -
ZoneName $DNSZone
#both up
